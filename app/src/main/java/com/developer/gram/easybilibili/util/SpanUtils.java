...
public final class SpanUtils {
    ...
    private void updateCharCharSequence() {
        ...
        if (iconMarginGapWidth != -1) {
            if (iconMarginBitmap != null) {
                ...
                mBuilder.setSpan(new CustomIconMarginSpan(EasyBiliApp.getInstance(), iconMarginUri, iconMarginGapWidth, alignIconMargin), start, end, flag);
            } else if (iconMarginResourceId != -1) {
                mBuilder.setSpan(new CustomIconMarginSpan(EasyBiliApp.getInstance(), iconMarginResourceId, iconMarginGapWidth, alignIconMargin), start, end, flag);
            }
        }
    ...
    private void updateImage() {
        ...
        int end = start + 5;
        if (imageBitmap != null) {
            mBuilder.setSpan(new CustomImageSpan(EasyBiliApp.getInstance(), imageBitmap, alignImage), start, end, flag);
        } else if (imageDrawable != null) {
            mBuilder.setSpan(new CustomImageSpan(imageDrawable, alignImage), start, end, flag);
        } else if (imageUri != null) {
            mBuilder.setSpan(new CustomImageSpan(EasyBiliApp.getInstance(), imageUri, alignImage), start, end, flag);
        } else if (imageResourceId != -1) {
            mBuilder.setSpan(new CustomImageSpan(EasyBiliApp.getInstance(), imageResourceId, alignImage), start, end, flag);
        }
    }
